...
Our KNN model uses all of the parameters from our training data and predicts the weather type.  We also found the misclassification rate, and displayed a confusion matrix, showing the predicted scores and the actual scores. 
...
Our K-means model for weather type is much less accurate, as can be seen with the table. We used all of the quantitative parameters to predict weather type, as in our KNN model.
## K-means
```{r}
library(fpc)
library(flexclust)
...
We used time series to predict the quantitative variables, which are temperature, windspeed, air pressure, and humidity.  The data has some obvious outliers, so we removed them from our initial plot   For all of our time series models, we first created vectors and performed time series analysis on those vectors.  We plotted each one with the outliers avergeraged out. Then, we used the HoltWinters method and the forecast method to predict the future weather.  To verify our time series model, we compared the actual values to our predicted values.  
#Time Series 
```{r}
#PREDICT TEMPERATURE
...
data.df$temp[dateRow:dateRow+120]
```
